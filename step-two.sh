...
if [[ $answer == "yes" ]];then
    ...
    # set fast mirror
    autoline
    echo "set fast mirror......."
    list=`grep -A 1 China /etc/pacman.d/mirrorlist`
    echo $list | grep Server | while read line ;do sed -i "1i ${line}\n" /etc/pacman.d/mirrorlist;done
...
127.0.1.1 Archlinux.localdomain Archlinux" > /etc/hosts
    ...
    if ls /sys/firmware/efi/efivars &>/dev/null;then
        ...
        echo "grub will be installed in `say /dev/${device}. agree?` yes/no"
        read confirm
        if [[ $confirm == "yes" ]];then
